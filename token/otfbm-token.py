...
def lambda_handler(event, context):
    ...
    with io.BytesIO() as buffer:
        ...
        shortCode = short_url.encode(number)

        contents = f'<html><head><title>Token ID for {imgUrl}</title><meta name="description" content="{shortCode}"></head><body>{shortCode}</body></html>'
        body = contents.encode()

        s3_client.put_object(Body=body, Bucket=bucket, Key=f'meta/{imgUrlEnc}',
                             ExtraArgs={'CacheControl': 'max-age=3600', 'ContentType': 'text/html'})
