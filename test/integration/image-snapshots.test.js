...
test("tokens", async () => {
  ...
  const canvas = await drawCanvas(`/${tokens.join("/")}`);
  expect(canvas.toBuffer("image/png")).toMatchImageSnapshot();
});

test("overlays", async () => {
  const overlays = ["A1$T", "B1$P", "C1$p", "D1$S", "E1$F", "F1$O", "G1$o"];
  const canvas = await drawCanvas(`/${overlays.join("/")}`);
  expect(canvas.toBuffer("image/png")).toMatchImageSnapshot();
});
...
test("walls", async () => {
  ...
  const canvas = await drawCanvas(`/${walls}`);
  expect(canvas.toBuffer("image/png")).toMatchImageSnapshot();
});

test("effects", async () => {
  const effects = ["*s15cc1c5", "*c20rh3", "*l40,2ya6j6", "*t15f10f3"];
  const canvas = await drawCanvas(`/${effects.join("/")}`);
  expect(canvas.toBuffer("image/png")).toMatchImageSnapshot();
});
...
test("background", async () => {
  ...
  const canvas = await drawCanvas(`/24x17`, { bg });
  expect(canvas.toBuffer("image/png")).toMatchImageSnapshot();
});

test("settings: dark mode", async () => {
  const canvas = await drawCanvas(`/@d`);
  expect(canvas.toBuffer("image/png")).toMatchImageSnapshot();
});

test("settings: zoom: 2", async () => {
  const canvas = await drawCanvas(`/@2`);
  expect(canvas.toBuffer("image/png")).toMatchImageSnapshot();
});

test("settings: zoom: 3", async () => {
  const canvas = await drawCanvas(`/@3`);
  expect(canvas.toBuffer("image/png")).toMatchImageSnapshot();
});

test("settings: grid transparency: half", async () => {
  const canvas = await drawCanvas(`/@h`);
  expect(canvas.toBuffer("image/png")).toMatchImageSnapshot();
});

test("settings: grid transparency: no grid", async () => {
  const canvas = await drawCanvas(`/@n`);
  expect(canvas.toBuffer("image/png")).toMatchImageSnapshot();
});
...
test("complex: multiple features 1", async () => {
  ...
  const bg =
    "https://cdn.discordapp.com/attachments/687568111498821642/714239512741412974/battlemap-3.png";
  const settings = "@2dn";
  const canvas = await drawCanvas(`/${tokens.join("/")}/${settings}`, { bg });
  expect(canvas.toBuffer("image/png")).toMatchImageSnapshot();
});
...
test("dotted lines: top right -> right & bottom left -> left", async () => {
  const bg =
    "https://cdn.discordapp.com/attachments/687568111498821642/714239512741412974/battlemap-3.png";
  const canvas = await drawCanvas(`/`, { bg });
  expect(canvas.toBuffer("image/png")).toMatchImageSnapshot();
});
...
test("dotted lines: top right -> up (pan) & bottom left -> left (pan)", async () => {
  const bg =
    "https://cdn.discordapp.com/attachments/687568111498821642/714239512741412974/battlemap-3.png";
  const canvas = await drawCanvas(`/o8:10x10`, { bg });
  expect(canvas.toBuffer("image/png")).toMatchImageSnapshot();
});
...
test("dotted lines: pan x only", async () => {
  const bg =
    "https://cdn.discordapp.com/attachments/687568111498821642/714239512741412974/battlemap-3.png";
  const canvas = await drawCanvas(`/c1:10x10`, { bg });
  expect(canvas.toBuffer("image/png")).toMatchImageSnapshot();
});
...
test("dotted lines: pan x only - to the end", async () => {
  const bg =
    "https://cdn.discordapp.com/attachments/687568111498821642/714239512741412974/battlemap-3.png";
  const canvas = await drawCanvas(`/q1:10x10`, { bg });
  expect(canvas.toBuffer("image/png")).toMatchImageSnapshot();
});
...
test("dotted lines: pan y only", async () => {
  const bg =
    "https://cdn.discordapp.com/attachments/687568111498821642/714239512741412974/battlemap-3.png";
  const canvas = await drawCanvas(`/a7:10x10`, { bg });
  expect(canvas.toBuffer("image/png")).toMatchImageSnapshot();
});

test("edge case: border between z and aa", async () => {
  const canvas = await drawCanvas(`/x1:5x5/z1/aa1`);
  expect(canvas.toBuffer("image/png")).toMatchImageSnapshot();
});

test("edge case: border between az and ba", async () => {
  const canvas = await drawCanvas(`/ax1:5x5/az1/ba1`);
  expect(canvas.toBuffer("image/png")).toMatchImageSnapshot();
});

test("edge cases: overlays", async () => {
  const overlays = ["aa1$T", "ab1$P", "ac1$p", "ad1$S", "ae1$F", "af1$O", "ag1$o"];
  const canvas = await drawCanvas(`/x1:10x10/${overlays.join("/")}`);
  expect(canvas.toBuffer("image/png")).toMatchImageSnapshot();
});
...
test("edge cases: walls", async () => {
  ...
  const canvas = await drawCanvas(`/aa1:10x10/${walls}`);
  expect(canvas.toBuffer("image/png")).toMatchImageSnapshot();
});

test("edge cases: effects", async () => {
  const effects = ["*s15cc1ac5", "*c20rah3", "*l40,2ya6aj6", "*t15f10af3"];
  const canvas = await drawCanvas(`/aa1:10x10/${effects.join("/")}`);
  expect(canvas.toBuffer("image/png")).toMatchImageSnapshot();
});
